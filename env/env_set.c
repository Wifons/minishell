...
static  t_env_var *create_env_var(const char *name, const char *value)
{
    ...
    if (value)
    {
        ...
        var->value = NULL;  // Set to NULL for no value, not empty string
    return (var);
}
...
static int  update_existing_var(t_env_var *var, const char *value)
{
    ...
    // If value is NULL (no = sign), only update if current value is also NULL
    // This implements bash behavior where "export test" doesn't override existing values
    if (!value)
    {
        if (var->value == NULL)
            return (0);  // Already NULL, nothing to do
        else
            return (0);  // Has a value, don't change it
    }
    
    // Update to new value (including empty string)
    free(var->value);
    var->value = NULL;
